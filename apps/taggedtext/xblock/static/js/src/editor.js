TaggedText.StudioView = function(runtime, element, server) {
    this.runtime = runtime;
    this.server = server;

    // Initialize the code box
    this.codeBox = CodeMirror.fromTextArea(
        $(element).find('.taggedtext-editor').first().get(0),
        {mode: "xml", lineNumbers: true, lineWrapping: true}
    );

    // Install click handlers
    var view = this;
    $(element).find('.taggedtext-save-button').click(
        function(eventData) {
            view.save();
    });

    $(element).find('.taggedtext-cancel-button').click(
        function(eventData) {
            view.cancel();
    });
};
